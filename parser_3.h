...
void Node::printMNA(ofstream& outFile, int datum, int lastnode) {
	...
	while (conPtr != NULL) {
		if (conPtr->comp->getType() == VSource) {  //~> seeking for a VSource
			if((conPtr->comp->con0.node->getNameNum()==0 && conPtr->comp->con1.node->getNameNum()!=0)
			|| (conPtr->comp->con0.node->getNameNum()!=0 && conPtr->comp->con1.node->getNameNum()==0)){
				print = 1;
				outFile << endl;
			...
			if( (conPtr->comp->con0.node->getNameNum()!=0 && conPtr->comp->con1.node->getNameNum()!=0)){
				print = 1;
				outFile << endl;
				if(this->getNameNum() == conPtr->comp->con1.node->getNameNum()){
					outFile << "F(" << conPtr->comp->con1.node->getNameNum() << ")=";
				}
				else{
					outFile << "F(" << lastnode + conPtr->comp->getcompNum() << ")=";
				}
	...
	while (conPtr != NULL) {
		if (conPtr->comp->getType() == VSource) {
			...
			if( (conPtr->comp->con0.node->getNameNum()!=0 && conPtr->comp->con1.node->getNameNum()!=0)){
					...
					//outFile << endl << "JAC(" << lastnode + val;
					if(this->getNameNum() == conPtr->comp->con1.node->getNameNum()){
						outFile << endl << "JAC(" << conPtr->comp->con1.node->getNameNum();
						flag1 = 1;
						num = conPtr->comp->con1.node->getNameNum();
					}
					else{
						outFile << endl << "JAC(" << lastnode + val;
					}
...
void Component::printMat(int nodeNum, int datum, int lastnode, double result[], int nameNum) {
	...
	// 		if (con0.node->getNameNum() != datum && con1.node->getNameNum() != datum) {
	// 			result[nameNum] = result[nameNum] + value*((nodeValue[con0.node->getNameNum()] - nodeValue[con1.node->getNameNum()]- preU[con0.node->getNum()]) / stepSize);
	// 		}
	// 		if (con0.node->getNameNum() != datum && con1.node->getNameNum() == datum) {
	// 			result[nameNum] = result[nameNum] + value*((nodeValue[con0.node->getNameNum()]-preU[con0.node->getNum()]) / stepSize);
	// 		}
	// 		if (con0.node->getNameNum() == datum && con1.node->getNameNum() != datum) {
	// 			result[nameNum] = result[nameNum] + value*((-nodeValue[con1.node->getNameNum()]+ preU[con0.node->getNum()]) / stepSize);
	// 		}
	// 	}
	// 	break;
	case Capacitor:
		if (!isTran) {
			...
			if (con0.node->getNum() == nodeNum) {
				result[nameNum] = result[nameNum] + value / stepSize * (nodeValue[con0.node->getNameNum()] - nodeValue[con1.node->getNameNum()] - temp_nodeValue[con0.node->getNameNum()]);
	...
	case Inductor:
		if (!isTran) {
			...
			if (con0.node->getNum() == nodeNum) {
				...
				if (con0.node->getNameNum() != datum && con1.node->getNameNum() != datum) {
					result[nameNum] = result[nameNum] + stepSize * (nodeValue[con0.node->getNameNum()] - nodeValue[con1.node->getNameNum()]) / value - temp_I[compNum];// - temp_I[compNum] //+ Icc[0][con0.node->getNum()]
				}
			...
			if (con1.node->getNum() == nodeNum) {
				...
				if (con1.node->getNameNum() != datum && con0.node->getNameNum() != datum) {
					result[nameNum] = result[nameNum] + stepSize * (nodeValue[con1.node->getNameNum()] - nodeValue[con0.node->getNameNum()]) / value + temp_I[compNum];// + temp_I[compNum] //+ Icc[1][con1.node->getNum()]
				}
		...
		// case Inductor:
		// if (con0.node->getNum() == nodeNum) {
		// 	// 根据向后欧拉方法离散化电感器模型，添加以下方程
		// 	result[nameNum] = result[nameNum] + (nodeValue[con0.node->getNameNum()] - nodeValue[con1.node->getNameNum()] - value * (nodeValue[con0.node->getNameNum()] - nodeValue[con1.node->getNameNum()]) / stepSize) / value;
		// }
		// if (con1.node->getNum() == nodeNum) {
		// 	// 根据向后欧拉方法离散化电感器模型，添加以下方程
		// 	result[nameNum] = result[nameNum] + (nodeValue[con1.node->getNameNum()] - nodeValue[con0.node->getNameNum()] - value * (nodeValue[con1.node->getNameNum()] - nodeValue[con0.node->getNameNum()]) / stepSize) / value;
		// }
		// break;
...
void Node::printMNAMat(int datum, int lastnode, double result[]) {
	...
	while (conPtr != NULL) {
		if (conPtr->comp->getType() == VSource) {  //~> seeking for a VSource
			...
			if( (conPtr->comp->con0.node->getNameNum()!=0 && conPtr->comp->con1.node->getNameNum()!=0)){
				if(this->getNameNum() == conPtr->comp->con1.node->getNameNum()){
					numIndex = conPtr->comp->con1.node->getNameNum();
				}
				else{
					numIndex = lastnode + conPtr->comp->getcompNum();
				}
	...
	while (conPtr != NULL) {
		if (conPtr->comp->getType() == VSource) {
			...
			if( (conPtr->comp->con0.node->getNameNum()!=0 && conPtr->comp->con1.node->getNameNum()!=0)){
					//outFile << endl << "JAC(" << lastnode + val;
					if(this->getNameNum() == conPtr->comp->con1.node->getNameNum()){
						fristIndex = conPtr->comp->con1.node->getNameNum();
						flag1 = 1;
						num = conPtr->comp->con1.node->getNameNum();
					}
					else{
				...
				|| (conPtr->comp->con0.node->getNameNum()==0 && conPtr->comp->con1.node->getNameNum()!=0)){
					if(flag1){
						if (conPtr->comp->getcompNum() == val)
							jacMat[num][lastnode + conPtr->comp->getcompNum()] = 1;
						else 
							jacMat[num][lastnode + conPtr->comp->getcompNum()] = 0;
					}
					else{
						if (conPtr->comp->getcompNum() == val)
